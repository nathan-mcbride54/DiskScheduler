...
// Created By: Nathan McBride (0615415)
// COIS-3320 Fundamentals of Operating Systems Lab 4

#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>

#define QueueSize 50000 // Max number of track requests possible

int RandomNumber(int randMax); // Used to generate a random number for track location
void FillQueue(int schedule_type, int seed, int max_requests); // Called to fill the queue with track requests from files
void AddToQueue(int trackRequest, int schedule_type); // Adds an individual track request to queue based on schedule type
void ShowResults(float result, int test_value, int schedule_type); // Outputs results generated by simulation
float ProcessQueue(void); // Returns the average head move for a given scheduled queue
int RemoveFromQueue(void); // Removes the track request at the head of the queue
void AutoSimulation(int schedule_type); // Created to run the simulation with 5 predefined seeds for each test value

int seeds[5] = {57, 123, 654, 23, 74}; // Seeds used for random number generation
int test_values[4] = {10, 100, 1000, 10000}; // Number of files to schedule track requests for
long Queue[QueueSize]; // Array to store track requests in queue fashion
int requestsInQueue; // Number of requests in the queue
int totalRequests; // Total requests to be processed
int numberFileRequests; // Number of file requests to be processed
int totalHeadMove; // Distance traveled by read head for a given processing of the queue
int currentTrack; // Current track of the read head
...
void AutoSimulation(int schedule_type) {
    float result;
    for(int j = 0; j <= 3; j++ ) {
        for(int i = 0; i <= 4; i++) {
            FillQueue(schedule_type, seeds[i], test_values[j]);
            result += ProcessQueue();
        }
        result = result / 5;
        ShowResults(result, test_values[j], schedule_type);
    }
}

void ShowResults(float result, int test_value, int schedule_type) {
    printf("===================================\n");
...
void FillQueue(int schedule_type, int seed, int max_requests) {
    Initialize(max_requests); // Clear global variables and flush queue
    srand(seed); // Seed random number generation
    while(numberFileRequests <= max_requests) { // Loop until the max requests is reached
        int numberTrackRequests = RandomNumber(5); // Randomly pick number of track requests
        for(int i = 0; i <= numberTrackRequests; i++ ) {
            int trackNumber = RandomNumber(799); // Randomly pick the track number
            AddToQueue(trackNumber, schedule_type); // Add the track request to queue
        }
 ...
 * (assume there are 10, 100, 1000, 10000 file requests). You then generate that number of track requests (each is a random number between 0 and799).
